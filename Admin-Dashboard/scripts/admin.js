...
async function loadCurrentSessions() {
    try {
        const response = await fetch(`${link}/session-settings`);
        const sessions = await response.json();

        const sessionsList = document.getElementById('sessions-list');
        sessionsList.innerHTML = ''; // Clear the existing list
...
async function deleteSession(sessionId) {
    ...
    try {
        const response = await fetch(`${link}/session-settings/${sessionId}`, {
            method: 'DELETE',
        });

        if (response.ok) {
            alert('Session deleted successfully');
            loadCurrentSessions(); // Reload session list after deletion
        } else {
